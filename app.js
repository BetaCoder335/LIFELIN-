(() => {
    ...
    async function startTriage(complaint = "", protocol = null) {
        try {
            ...
            const data = await backendFetch("/start", {
                method: "POST",
                body: JSON.stringify({ complaint, protocol })
            });
            state.session = data.session_id;
            renderQuestion(data.protocol_id, data.question, data.question_index, data.total_questions);
        } catch (error) {
    ...
    function renderQuestion(protocol, question, index = 0, total = 0) {
        $("triage-question-card")?.classList.remove("hidden");
        if ($("badge-proto-name")) $("badge-proto-name").textContent = protocol || "Triage";
        if ($("question-progress")) $("question-progress").textContent = total ? `Question ${index + 1} of ${total}` : "Assessment";
        if ($("triage-question-text")) $("triage-question-text").textContent = question?.text || "";
        ...
        (question?.options || []).forEach((option, i) => {
            ...
            label.innerHTML = `<input type="radio" name="triage-opt" value="${escapeAttr(option.id)}" ${i === 0 ? "checked" : ""}><span>${escapeHtml(option.label)}</span>`;
            list.appendChild(label);
        });

        list.querySelectorAll("input").forEach(input => {
            input.addEventListener("change", () => {
                list.querySelectorAll(".triage-option").forEach(opt => opt.classList.remove("selected"));
                input.closest(".triage-option")?.classList.add("selected");
            });
        });
        list.querySelector(".triage-option")?.classList.add("selected");
    }

    async function submitAnswer() {
        const selected = document.querySelector('input[name="triage-opt"]:checked');
        if (!selected || !state.session) return;

        try {
            const data = await backendFetch("/answer", {
                method: "POST",
                body: JSON.stringify({ session_id: state.session, option_id: selected.value })
            });
    ...
    function renderResult(result) {
        ...
        if (banner) {
            if (severity === "red") { banner.style.background = "#fff0f1"; banner.style.color = "#c53d3d"; }
            else if (severity === "orange") { banner.style.background = "#fff7e8"; banner.style.color = "#cf6a23"; }
            else { banner.style.background = "#e8f8ef"; banner.style.color = "#0a8f55"; }
        }
        ...
        if (list) {
            ...
            (result.actions || []).forEach(action => {
                ...
                el.textContent = action.text || action.label || String(action);
                list.appendChild(el);
            });
        }
